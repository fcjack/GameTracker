...
func TestEnsureFreshTokensReturnsExistingAccessToken(t *testing.T) {
	...
	user, err := models.CreateUser(ctx, db, fmt.Sprintf("epic_tokens_%d", time.Now().UnixNano()), "password123")
	if err != nil {
	...
	if _, err := models.UpsertLinkedAccount(
		ctx, db, user.ID, "epic", "9626f441055349ce8cb7d7d5a483eaa2", "EpicGamer",
...
func TestEnsureFreshTokensRefreshesExpiredToken(t *testing.T) {
	...
	user, err := models.CreateUser(ctx, db, fmt.Sprintf("epic_refresh_%d", time.Now().UnixNano()), "password123")
	if err != nil {
		t.Fatalf("CreateUser() error = %v", err)
	}
...
func TestEnsureFreshTokensRevokedRefreshToken(t *testing.T) {
	db := testDB(t)
	defer db.Close()
	...
	user, err := models.CreateUser(ctx, db, fmt.Sprintf("epic_revoked_%d", time.Now().UnixNano()), "password123")
	if err != nil {
		t.Fatalf("CreateUser() error = %v", err)
	}
	...
	refreshEnc, err := enc.Encrypt("revoked-refresh-token")
	if err != nil {
		t.Fatalf("Encrypt(refresh) error = %v", err)
	}
	...
	_, err = EnsureFreshTokens(ctx, client, enc, db, user.ID)
	if err == nil {
		t.Fatal("EnsureFreshTokens() error = nil, want revoked refresh failure")
	}
	if !strings.Contains(err.Error(), "Refresh token expired or revoked") {
		t.Errorf("EnsureFreshTokens() error = %v, want revoked refresh details", err)
	}
}
