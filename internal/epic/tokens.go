...
// EnsureFreshTokens loads the user's linked Epic account, refreshes OAuth tokens
...
func EnsureFreshTokens(
	...
	account, err := models.GetLinkedAccount(ctx, db, userID, "epic")
	if err != nil {
		if err == pgx.ErrNoRows {
			return nil, fmt.Errorf("epic: account not linked")
		}
		return nil, err
	}
	...
	if err != nil {
		return nil, fmt.Errorf("epic: decrypt access token: %w", err)
	}
	...
	if account.RefreshTokenEnc != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("epic: decrypt refresh token: %w", err)
		}
	}
	...
	if refreshToken == "" {
		return nil, fmt.Errorf("epic: refresh token missing")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("epic: encrypt access token: %w", err)
	}
	...
	if tokens.RefreshToken != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("epic: encrypt refresh token: %w", err)
		}
	}
